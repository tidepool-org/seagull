...
describe('userapi', function(){
  ...
  describe('GET /collections', function() {
    it('should respond with 200, profile', function(done) {
      supertest
        .get('/collections')
        .expect(200)
        .end(function(err, obj) {
           if (err) return done(err);
           expect(err).to.not.exist;
           expect(obj.res.body.length).to.eql(1);
           expect(obj.res.body[0]).to.eql('profile');
           done();
         });
    });

  // POST /:metaid/:collection -- creates a block of metadata or fails if collection exists for that metaid
  // GET /:metaid/:collection -- returns the top-level metadata information
  // PUT /:metaid/:collection -- updates metadata
  // DELETE /:metaid/:collection -- deletes the metaid record
  //
  // POST /:metaid/group/:name -- creates a group of the given name, assigns group ID
  // GET /:metaid/group/:name -- returns group ID
  // PUT /:metaid/group/:name -- updates group ID
  // DELETE /:metaid/group/:name -- deletes the name from the groups list
  //
  // POST /:metaid/private/:name -- creates a private id/hash of the given name
  // GET /:metaid/private/:name -- returns private id/hash
  // PUT /:metaid/private/:name -- updates private id/hash
  // DELETE /:metaid/private/:name -- deletes the name from the privates list
