...
function createServer(serverConfig, crudHandler, userApiClient, gatekeeperClient, metrics) {
  ...
  var checkTokenInternal = userApi.middleware.checkToken(userApiClient, "tidepool:internal");
  var checkTokenPublic = userApi.middleware.checkToken(userApiClient, "tidepool:public");
  ...
  // unified users
  retVal.get('/users/:userid/users', checkTokenPublic, permissions.requireCustodian, seagullApi.users);

  // get the valid collections
  retVal.get('/collections', seagullApi.metacollections);

  // manage the private information
  retVal.get('/:userid/private/:name', checkTokenPublic, permissions.requireServer, seagullApi.metaprivate_read);
  retVal.del('/:userid/private/:name', checkTokenInternal, permissions.requireServer, seagullApi.metaprivate_delete);
  ...
  retVal.post('/:userid/:collection', checkTokenInternal, permissions.requireCustodian, seagullApi.metacollection_update);
  retVal.get('/:userid/:collection', checkTokenPublic, permissions.requireMembership, seagullApi.metacollection_read);
  retVal.put('/:userid/:collection', checkTokenInternal, permissions.requireCustodian, seagullApi.metacollection_update);
  retVal.del('/:userid/:collection', checkTokenInternal, permissions.requireCustodian, seagullApi.metacollection_delete);
