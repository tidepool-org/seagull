...
function createServer(serverConfig, crudHandler, userApiClient, armadaClient) {
  ...
  var getMeta = require('./getMetaPair')(userApiClient, armadaClient);
  // var metrics = userApi.metrics.createMetrics(serverConfig, log);
  // var trackServer = _.partial(userApi.middleware.trackServer(metrics, 'seagull'));
...
module.exports = function seagullService(envConfig, crudHandler, userApiClient, armadaClient) {
  var server = null;
  ...
  if (envConfig.httpPort != null) {
    ...
    server = createServer({ name: 'SeagullHttp' }, crudHandler, userApiClient, armadaClient);
  }

  if (envConfig.httpsPort != null) {
    servicePort = envConfig.httpsPort;
    server = createServer(_.extend({ name: 'SeagullHttps' }, envConfig.httpsConfig), crudHandler, userApiClient ,armadaClient);
  }
