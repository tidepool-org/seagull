...
*
***/
var checkUserPermissions = function(request, client, cb){
  if (request._tokendata.isserver) {
    return cb(null, request.params.userid);
  } else if ( request._tokendata.userid != request.params.userid) {

    /*
      TODO: check (using the amardaClient) that req.params.userid
      is in req._tokendata.userid 'patients' or 'team' group
    */

    return cb(null,request.params.userid);
  }
  return cb(null, request._tokendata.userid);
};
...
module.exports = function(userClient, armadaClient) {
  return function(req, res, next) {
    ...
    checkUserPermissions(req, armadaClient, function(permissionsError,userid){
