...
(function () {
  ...
  var mongoConfig = {
    ...
    mongoConnectionString: config.mongoDbConnectionString
  };

  var discoveryConfig = config.discovery;
  var hakkenClient = require('hakken')(discoveryConfig).client.make();
  hakkenClient.start();

  var hostGetter = hakkenClient.randomWatch(config.userApi.serviceName, null, { log: logMaker('user-api-watch') });
  hostGetter.start();
