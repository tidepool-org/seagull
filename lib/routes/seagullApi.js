...
module.exports = function (crudHandler, userApiClient) {
  ...
  return {
    ...
    publicinfo_get: function(req, res, next) {
      log.debug('publicinfo_get: params[%j], url[%s], method[%s]', req.params, req.url, req.method);

      console.log(req._tokendata);
      if (req._tokendata.isserver) {
        res.send(400, 'User token required.');
        return next();
      }

      var users = req.params.users;
      if (users == null) {
        res.send(400, 'No users specified');
        return next();
      }

      var userids = users.split(',');
      console.log(userids);
      // for each user, look up the user info in the user-api
      // Helper function so we can do async.map.
      var getuserinfo = function(id, callback) {
        userApiClient.getUserInfo(id, function(err, reply) {
          if (err) {
            callback(null, {userid: id, error: 'invalid id'});
          } else {
      ...
      async.map(userids, getuserinfo, function(err, results) {
        if (err) {
          res.send(500, 'something went wrong in getuserinfo!');
        } else {
          res.send(results);
        }
      });
