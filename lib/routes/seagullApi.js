      ...
      gatekeeperClient.groupsForUser(targetUserId, function(error, trustorUserPermissions) {
        ...
        gatekeeperClient.usersInGroup(targetUserId, async function(error, trusteeUserPermissions) {
          ...
          try {
            ...
            if (!_.isEqual(_.map(userProfiles, 'userid').sort(), _.sortBy(userIds))) {
              throw new Error('Received less results than we queried for');
            }
          } catch (error) {
            res.send(500);
            return next(false);
            ...
            crudHandler.getDoc(trustorUserId, function(error, document) {
              ...
              if (_.isEmpty(user.trustorPermissions)) {
                if (user.profile) {
                  ...
                  log.error(`User ${user.userid} does not have a valid profile. Consider investigaing the account.`);
                }
              } else {
                if (
