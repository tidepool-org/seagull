...
module.exports = function (crudHandler, userApiClient, gatekeeperClient, metrics) {
  ...
  function createDocAndCallback(userId, res, next, cb) {
    log.info( "createDocAndCallback userId %s", userId);
  ...
  function getCollection(req, res, sanitize, next) {
    crudHandler.getDoc(_.get(req, 'params.userid'), function (err, result) {
      if (err) {
        ...
        if (retVal == null) {
          log.info( "getCollection empty collection %s", collection);
          res.send(404);
        } else {
          log.info( "getCollection found collection %s", collection);
  ...
  return {
    ...
    metacollection_update: function (req, res, next) {
      log.info('metacollection_update: params[%j], url[%s], method[%s]', req.params, req.url, req.method);
      ...
      function doUpdate(addIfNotThere) {
        ...
        log.info("ENTER doUpdate addIfNotThere %s userId %s updates %s", addIfNotThere, userId, JSON.stringify(updates));
        crudHandler.partialUpdate(userId, updates, function (err, result) {
          log.info("ENTER process partialUdpate");
          if (err) {
            if (err.statusCode == 404 && addIfNotThere) {
              log.info("doUpdate userId %s: 404 and addifNotThere", userId);
              return createDocAndCallback(userId, res, next, function () { doUpdate(false); });
            } else {
              log.info("doUpdate userId %s: other err %s", userId, err);
              log.error(err, 'Error updating metadata doc');
              res.send(err.statusCode);
              return next();
            }
          } else {
            log.info("doUpdate userId %s: success", userId);
            res.send(200, result.detail[collection]);
            return next();
          }
          log.info("EXIT process partialUdpate");
        });
        log.info("EXIT doUpdate addIfNotThere %s userId %s updates %s", addIfNotThere, userId, JSON.stringify(updates));
      }
