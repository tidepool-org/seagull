    ...
    users: function(req, res, next) {
      ...
      gatekeeperClient.groupsForUser(targetUserId, function(error, trustorUserPermissions) {
        ...
        gatekeeperClient.usersInGroup(targetUserId, async function(error, trusteeUserPermissions) {
          ...
          var userIds;
          try {
            userIdsWithDups = _.keys(mergedUserPermissions);
            ...
            const expectedIds = userProfiles.map( v => v.userid );
            const droppedIds = expectedIds.filter(v => !userIds.includes(v));
            const droppedProfiles = userProfiles.filter(v => droppedIds.includes(v));
            log.error(error, 'Failed to retreive these users', droppedIds );
            log.error(error, 'Failed to retreive these profiles', droppedProfiles );
            res.send(500);
            return next(false);
          }
