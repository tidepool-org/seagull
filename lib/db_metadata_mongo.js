...
define(['lodash', 'mongojs', 'crypto-js'], function(_,  mongojs, crypto) {
  ...
  function create_passphrase(pair) {
  ...
  function _encrypt_value(pair, value) {
    return encryption_algorithm.encrypt(value, create_passphrase(pair));
  }

  function _decrypt_value(pair, value) {
    return encryption_algorithm.decrypt(value, create_passphrase(pair));
  }
  ...
  function getDoc(pair, done) {
    ...
    db.seagull.findOne({_id: pair.id}, {w:1}, function(err, result) {
      if (err) {
        done({statuscode: 404, message: 'not found', detail: err}, null);
      } else {
        var decrypted = _cleanupData(_decrypt_value(pair, result.value));
  ...
  function _updateDoc(pair, value, done) {
    ...
    db.seagull.update({_id: pair.id}, {value: crypted}, {w:1}, function(err, result) {
      if (err) {
        ...
        done(null, {statuscode: 201, message: 'created', detail: value});
      }
    });
  }

  function apply_updates(item, updates) {
