...
module.exports = function init(config) {
  ...
  function createDoc(userId, value, done) {
    ...
    db.seagull.insert({userId: userId, value: crypted}, { writeConcern: {w: 1}}, function (err, result) {
      if (err) {
  ...
  function getDoc(userId, done) {
    log.info("ENTER: _getDoc userId %s", userId);
    db.seagull.findOne({userId: userId}, {value: 1}, function (err, result) {
      log.info("findOne returns: err %s, result %s", err, result);
      if (err || (result === null)) {
        ...
        if (decrypted) {
          log.info("getDoc userId %s: found, value %s", userId, decrypted);
    ...
    log.info("EXIT: _getDoc userId %s", userId);
  }
  ...
  function _updateDoc(userId, value, done) {
    ...
    db.seagull.update({userId: userId}, {$set: {value: crypted}}, { writeConcern: {w: 1}}, function (err, result) {
      if (err) {
