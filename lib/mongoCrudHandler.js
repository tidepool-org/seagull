...
module.exports = function init(config) {
  ...
  const client = new mongodb.MongoClient(config.mongoConnectionString);
  const databaseName = getDatabaseName(config.mongoConnectionString, 'seagull');
  const db = client.db(databaseName);

  const seagullCollection = db.collection('seagull');
  seagullCollection.createIndex({userId: 1}, {unique: true, background: true}).catch(console.error);
  ...
  var ourexports = {
    ...
    deleteAllDocumentsForUser: deleteAllDocumentsForUser,
  };
  ...
  function createDoc(userId, value, done) {
    ...
    seagullCollection.insertOne({userId: userId, value: crypted}, {w: "majority"}, function (err, result) {
      if (err) {
  ...
  function getDoc(userId, done) {
    ...
    seagullCollection.findOne({userId: userId}, {value: 1}, function (err, result) {
      if (err || (result === null)) {
  ...
  function _updateDoc(userId, value, done) {
    ...
    var crypted = _encrypt_value(value);
    seagullCollection.updateOne({userId: userId}, {$set: {value: crypted}}, {w: "majority"}, function (err, result) {
      if (err) {
  ...
  function deleteAllDocumentsForUser(userId, done) {
    seagullCollection.deleteMany({userId: userId}, {}, done);
  }

  // this is a special function that is used in testing
  function _wipeTheEntireDatabase(done) {
    const listCollections = util.callbackify(() => {
      return db.listCollections({}, { nameOnly: true }).toArray();
    });

    listCollections((err, result) => {
      if (err) {
        log.error('getCollectionNames failed');
        done(err, null);
      } else {
        if (result.length !== 0) {
          seagullCollection.drop(function(err) {
            if (err) {
              done(err);
            } else {
              seagullCollection.createIndex({userId: 1}, {unique: true}, done);
            }
          });
        }
        else {
