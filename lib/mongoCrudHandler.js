...
module.exports = function init(config) {
  ...
  function createDoc(userId, value, done) {
    ...
    log.info("ENTER: createDoc userId %s, value %s", userId, crypted);
    db.seagull.insert({userId: userId, value: crypted}, {w: "majority"}, function (err, result) {
      log.info("ENTER insert continuation");
      if (err) {
        if (err.code == 11000) {    // this is a mongo error for duplicate key
          log.info("createDoc userId %s: duplicate key", userId);
          done({statusCode: 400, message: 'duplicate key', detail: err}, null);
        } else {
          log.info("createDoc userId %s: other mongo err %s", userId, err);
          done(err, null);
        }
      } else {
        // do we want to do something with result here?
        log.info("createDoc userId %s: created", userId);
        done(null, {statusCode: 201, message: 'created', detail: _cleanupData(value)});
      }
      log.info("EXIT insert continuation");
    });
    log.info("EXIT: createDoc userId %s, value %s", userId, crypted);
  }

  function getDoc(userId, done) {
    log.info("ENTER: getDoc userId %s", userId);
    // findOne runs asynchrounously
    db.seagull.findOne({userId: userId}, {value: 1}, function (err, result) {
      log.info("ENTER findOne continuation");
      if (err || (result === null)) {
        log.info("getDoc userId %s: not found ", userId);
        done({statusCode: 404, message: 'not found', detail: err}, null);
      } else {
        var decrypted = _decrypt_value(result.value);
        if (decrypted) {
          log.info("getDoc userId %s: found, value %s", userId, result.value);
          done(null, {statusCode: 200, message: 'found', detail: decrypted});
        } else {
          log.info("getDoc userId %s: not authorized", userId);
          done({statusCode: 401, message: 'not authorized', detail: null}, null);
        }
      }
      log.info("EXIT findOne continuation");
    });
    log.info("EXIT: getDoc userId %s", userId);
  }
  ...
  function _updateDoc(userId, value, done) {
    ...
    var crypted = _encrypt_value(value);
    log.info("ENTER: _updateDoc userId %s, value %s", userId, crypted);
    db.seagull.update({userId: userId}, {$set: {value: crypted}}, {w: "majority"}, function (err, result) {
      log.info("ENTER update continuation");
      if (err) {
        log.info("_updateDoc userId %s: server error", userId);
        done({statusCode: 500, message: 'server error', detail: err}, null);
      } else {
        log.info("_updateDoc userid %s: created", userId);
        done(null, {statusCode: 200, message: 'created', detail: value});
      }
      log.info("EXIT update continuation");
    });
    log.info("EXIT: _updateDoc userId %s, value %s", userId, crypted);
  }
  ...
  function partialUpdate(userId, updates, done) {
    ...
    getDoc(userId, function (err, result) {
      log.info("ENTER: partialUpdate/getDoc (read modify write)")
      if (err) {
        done(err, null);
      } else {
      ...
      log.info("EXIT: partialUpdate/getDoc (read modify write)")
    });
  }
